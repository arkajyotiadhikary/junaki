...
Upgrade the dependency-free stats dashboard (`stats.svg`, generated via `tsx scripts/generate-svg.ts`) with review and curriculum KPIs, track sections (system design, behavioral, mocks), a 26-week curriculum timeline bar, and a weighted MAANG readiness meter with caps, while preserving the existing visual style, build, and pre-commit regeneration flow.
