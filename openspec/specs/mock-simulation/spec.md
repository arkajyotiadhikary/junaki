...
Provide a timed mock-interview simulation mode that runs one interview session of a requested type (OA, staged, or AI-pair), drives a terminal countdown with audible warnings, selects eligible curriculum problems honoring an unaided-recency rule, and records each session outcome into the existing practice and spaced-repetition data so mock attempts flow through the same scheduler and stats as real solves.
